...
void Sequence_generator_node::Sequence_generator()
{
    ...
    RCLCPP_INFO(get_logger(), "Camera x: %f, Camera y: %f", pix_x0_, pix_y0_);
    RCLCPP_INFO(get_logger(), "theta0: %f, thetaf: %f", theta_z0_, theta_zf_);
    RCLCPP_INFO(get_logger(), "x0: %f, xf: %f", x0_, xf_);
    ...
    for (size_t i = 0; i < buffer_size; i++)
    {
        ...
        theta_calculated = a[0] + a[1] * t + a[2] * power(t, 2) + a[3] * power(t, 3);
        RCLCPP_INFO(get_logger(), "Time: %f, Twist angular.z: %f, Twist linear.x: %f", t, twist.angular.z, twist.linear.x);
        RCLCPP_INFO(get_logger(), "Calculated theta: %f", theta_calculated);
    }
}
